...
# 최종 쓰기 
CSV.open("C:/Users/ERC/Documents/김창현 개인/루비/지방공기업모델/result.csv", "w") do |csv|
    csv.to_io.write "\uFEFF"
    csv <<  ["구분","구분"] + Array.new($year){|n| n + $first_year } 
    csv <<  ["현행","영업이익"] + current_profit 
    csv <<  ["공단","영업이익"] + public_profit
    csv <<  ["수지개선","효과"] + suji
    csv <<  ["공단","경상수지비율"] + array_suji(make_line(public_sale),public_cost)
end